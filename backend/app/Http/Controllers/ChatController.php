...
class ChatController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $user = $request->user();
        $query = Chat::query()->with(['channel', 'externalUser', 'assignedOperator'])->with(['messages' => fn ($q) => $q->latest('id')->limit(1)]);
        ...
        $chats = $query->orderByRaw('CASE WHEN last_inbound_message_at IS NULL THEN 1 ELSE 0 END')
            ...
            ->limit($limit + 1)
            ->get();
        $next = $chats->count() > $limit ? $chats->last()->id : null;
        $items = $chats->take($limit)->map(fn (Chat $chat) => $this->presenter->chat($chat, $user));
    ...
    public function messages(Request $request, Chat $chat): JsonResponse
    {
        ...
        $next = $messages->count() > $limit ? $messages->last()->id : null;

        return response()->json(['data' => $messages->take($limit)->map(fn ($m) => $this->presenter->message($m))->values(), 'next_cursor' => $next]);
    }
    ...
    public function close(Request $request, Chat $chat): JsonResponse
    {
        $closed = DB::transaction(function () use ($chat, $request) {
            ...
            if ($locked->assigned_operator_id !== null && ! $request->user()->isAdmin() && ! $locked->isAssignedTo($request->user())) {
